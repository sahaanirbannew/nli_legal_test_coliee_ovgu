...
def sentence2sequence(sentence):
    ...
    for word in vocabulary:
        ...
        if word in law2vec_wordmap:
            ...
            rows.append(np.zeros((word_dimension)))  # hardcode for UNK token
        words.append(word)
    return rows, words
...
def get_data(preprocessed_json_file, datatype="TRAIN"):
    ...
    premise_sentences = np.stack([fit_to_size(sent, (max_premise_length, word_dimension))
                      for sent in premise_sentences])
    
    hyp_sentences = np.stack([fit_to_size(sent, (max_hypothesis_length, word_dimension))
