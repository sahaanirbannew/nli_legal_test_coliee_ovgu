# -*- coding: utf-8 -*-
"""
Created on Sun May 24 11:25:46 2020
...
def add_sentence_tags(premise, hyp, word_dim, tag_dim, sim_vectors):
    '''
    ...
    sim_vectors = np.reshape(sim_vectors, (sim_vectors.shape[0], 1, sim_vectors.shape[1]))              # reshape sim_vectors to a 3D matrix for maintaining dimensionality when concatenating
...
def get_data(preprocessed_json_file, datatype="TRAIN"):
    ...
    for _, pair in data.items():
        ...
        sim_vectors.append(np.stack(sim_neg + [0]*(word_dimension + tag_dimension - len(sim_neg))))
    ...
    print('Data parsing from JSON file: Complete')
    print('Data encoded to word embeddings: Complete')
    ...
    print('Premise & Hypothesis sentence padding: Complete')
    
    if datatype == "TRAIN":
        return add_sentence_tags(premise_sentences, hyp_sentences, word_dimension, tag_dimension, sim_vectors), labels
    else:
        return add_sentence_tags(premise_sentences, hyp_sentences, word_dimension, tag_dimension, sim_vectors)
