...
# regularizers to be applied separately for train & validation set
keep_prob = tf.placeholder(tf.float32, name='keep_prob')        # dropout regularizer 
weight_decay = tf.placeholder(tf.float32, name='weight_decay')  # weight decay value for l2 regularizer
tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, tf.nn.l2_loss(fc_weights['out']))  # collection of regularization losses
...
with tf.name_scope("loss"):
    ...
    reg_losses = tf.compat.v1.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)                    # apply regularizer to all weights   
            ...
            if epoch == 1 or epoch % display_step == 0:                             # print and save necessary information about training only at an interval of 'display_step' number of steps to reduce computational complexity
                ...
                if i == inner_split:                                                # last batch split of the selected epoch 
                    ...
                    if epoch == training_steps:                                 # do not change this intendation to make sure this line run only once and not for each split of the epoch!
                        ...
                        save_LSTM_states(statses_inter, state_val, SAVE_STATES_TO+'-final.hdf5')
